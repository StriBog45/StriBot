namespace StriBot
{
    static public class PriceList
    {
        static public int Song { get; } = 5;
        static public int Group { get; } = 10;
        static public int Hero { get; } = 20;
        static public int Anime { get; } = 20;
        static public int Movie { get; } = 80;
        static public int Halberd { get; } = 5;
        static public int Game { get; } = 80;
        static public int Cosplay { get; } = 200;
        static public int VIP { get; } = 300;
        static public int Boost { get; } = 120;
        static public int ToysForSub { get; } = 30;
    }
}